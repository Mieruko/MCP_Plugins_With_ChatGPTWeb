...
export function registerContextTools(server: McpServer, _workspaceRoot: string): void {
  server.registerTool(
    ...
    async ({ path: projectPath, max_depth, max_bytes_per_file }) => {
      const root = await validatePath(projectPath || getDefaultCwd());
      ...
      for (const file of files) {
        try {
          const buf = await fs.readFile(await validatePath(file));
  ...
  server.registerTool(
    ...
    async ({ path: filePath }) => {
      ...
      const rules = await loadPathRulesForFile(getDefaultCwd(), validPath);
