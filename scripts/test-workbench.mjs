...
try {
  ...
  assert.match(workbenchHtml, /id="tree-collapse"/);
  ...
  assert.match(appUi, /submit\.textContent = 'Adding…'/, 'workspace creation exposes a busy state');
  assert.match(appUi, /Workspace added · refresh failed:/, 'post-create refresh failure is not reported as creation failure');
  ...
  assert.equal(isolatedTask.execution.detached, true);
  ...
  assert.equal(isolatedOverview.status.branch, '');
  assert.equal(isolatedOverview.status.head_oid, git(['rev-parse', 'HEAD']));
  assert.ok(isolatedOverview.worktrees.some(item => path.resolve(item.path) === path.resolve(isolatedTask.execution.path) && item.detached));
  console.log('OK managed task worktree is detached, isolated and visible through Git overview');
  ...
  await fs.rm(resolved, { recursive: true, force: true, maxRetries: 8, retryDelay: 120 });
}
