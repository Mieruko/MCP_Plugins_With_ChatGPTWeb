...
**Switch an existing chat to another task in the same workspace:** in Advanced mode, ask Coder to call `workbench` to find the task, then `workbench_control(action=target, task_title="Kết nối data", create_missing=false)` (or use its exact `task_id`), then `project_context` without an explicit path. This scoped switch is permitted even when the current task is Ask/workspace-only: it only changes the caller's binding, never creates a task/workspace, never changes the selected dashboard task, and never changes another session. The old chat must have no pending/running operations or managed processes/preview; a target already owned by another chat, reserved for another session, or sharing a checkout with another active task is refused. Task names must be unique within the workspace or the caller must supply an exact task ID. Explicit cross-workspace targeting and workspace/task creation still require Full machine scope. Do not use `create_missing=true` to read a task that already exists. The switch response is authoritative only after success; verify the new task and reload its instructions with `project_context` before work. Restart the server and reconnect the ChatGPT connector after deploying changed MCP tool descriptions.
